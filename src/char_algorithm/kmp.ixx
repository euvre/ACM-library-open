...
export module kmp;
import acm_except;
// import acm_concept;// 试图简化模板？

namespace ACM {
// 已知缺陷：当 std::is_floating_v<std::remove_cvref_v<_Ty>> == true 时，程序可能无法达到预期的效果。
template<typename _Ty>
concept kmp_string_concept = std::ranges::sized_range<_Ty> && std::ranges::contiguous_range<_Ty> && requires(std::ranges::range_value_t<_Ty> _T_) { _T_ == _T_; };
